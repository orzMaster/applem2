...
type
  ...
  private
    ...
    function ZipFile(sDest, sDir: string): Boolean;
  public
    constructor Create();
      ...
      0: begin
          case m_wStatus of
            0: if IsBackUp then begin
                m_wStatus := 1;
                if m_sSourceDir[length(m_sSourceDir)] <> '\' then
                  m_sSourceDir := m_sSourceDir + '\';
                if m_sDestDir[length(m_sDestDir)] <> '\' then
                  m_sDestDir := m_sDestDir + '\';
...
var
  ...
  begin
    with Info do
      Result := (Name <> '.') and (Name <> '..') and ((attr and fadirectory) =
        fadirectory);
  end;
...
function TBackUpObject.ZipFile(sDest, sDir: string): Boolean;
begin
  try
    with m_Zip do begin
      ClearZip;
      ...
      RootDir := sDir;
      FilesList.Clear;
      FilesList.Add(sDir + '*.*'{m_BackUpFileList});
      Recurse := True;
      Zip;
    end;
    Result := True;
...
begin
  ...
  try
    for I := 0 to m_BackUpList.Count - 1 do begin
      if CompareText(TBackUpObject(m_BackUpList.Items[I]).m_sSourceDir, sSource)
        = 0 then begin
