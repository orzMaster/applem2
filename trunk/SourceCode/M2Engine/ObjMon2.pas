    ...
    if IsProperTarget(BaseObject) then begin
      if not BaseObject.m_boHideMode or m_boCoolEye then begin
        if (abs(m_nCurrX - BaseObject.m_nCurrX) < n554) and (abs(m_nCurrY -
          BaseObject.m_nCurrY) < n554) then begin
          sub_FFE9();
          break;
        end;
      end;
...
begin
  if n550 > 0 then begin
    if m_PEnvir.GetNextPosition(m_nCurrX, m_nCurrY, m_btDirection, 1, n08, n0C)
      then begin
  ...
  for i := 0 to m_VisibleActors.Count - 1 do begin
    ...
    if IsProperTarget(BaseObject) then begin
      if (abs(m_nCurrX - BaseObject.m_nCurrX) <= 1) and (abs(m_nCurrY -
        BaseObject.m_nCurrY) <= 1) then begin
        n10 := 0;
...
begin
  ...
  if m_Castle <> nil then begin
    if m_LastHiter = BaseObject then
      Result := True;
    //if BaseObject.m_TargetCret = Self then Result := True;
    if (BaseObject <> nil) and (BaseObject.bo2B0) then begin
      if (GetTickCount - BaseObject.m_dw2B4Tick) < 60 * 1000 then begin
        Result := True;
      end
      else
    ...
    if TUserCastle(m_Castle).m_MasterGuild <> nil then begin
      if BaseObject.m_Master = nil then begin
        if (TUserCastle(m_Castle).m_MasterGuild = BaseObject.m_MyGuild) or
          ...
          if (m_LastHiter <> BaseObject) and (not BaseObject.bo2B0) then
            Result := False;
        end else
        if (TUserCastle(m_Castle).m_MasterGuild <> BaseObject.m_MyGuild) and
          (TUserCastle(m_Castle).m_MasterGuild.IsWarGuild(TGuild(BaseObject.m_MyGuild))) then
          Result := True;
      end
      else begin //004A6988
        if (TUserCastle(m_Castle).m_MasterGuild = BaseObject.m_Master.m_MyGuild) or
          ...
          if (m_LastHiter <> BaseObject.m_Master) and (m_LastHiter <> BaseObject) and (not BaseObject.bo2B0) then
            Result := False;
        end else
        if (TUserCastle(m_Castle).m_MasterGuild <> BaseObject.m_Master.m_MyGuild) and
          (TUserCastle(m_Castle).m_MasterGuild.IsWarGuild(TGuild(BaseObject.m_Master.m_MyGuild))) then
          Result := True;
      end;
    end; //004A69EF
    if (BaseObject <> nil) and BaseObject.m_boAdminMode or BaseObject.m_boStoneMode or
      ((BaseObject.m_btRaceServer >= RC_NPC) and (BaseObject.m_btRaceServer < RC_ANIMAL)) or
  ...
  if (BaseObject.m_TargetCret <> nil) and (BaseObject.m_TargetCret.m_btRaceServer = RC_ARCHERGUARD) then
    Result := True;
  if (BaseObject <> nil) and (BaseObject.PKLevel >= 2) then
    Result := True;
  if (BaseObject <> nil) and (BaseObject.m_boAdminMode or BaseObject.m_boStoneMode or (BaseObject = Self) or (BaseObject.m_btRaceServer = RC_GUARD)) then
    Result := False;
end;
...
begin
  ...
  if not m_boDeath and not m_boGhost and CanWork then begin
    if (Integer(GetTickCount - m_dwWalkTick) >= m_nWalkSpeed) then begin
      m_dwWalkTick := GetTickCount();
      for i := 0 to m_VisibleActors.Count - 1 do begin
        BaseObject := TBaseObject(pTVisibleBaseObject(m_VisibleActors.Items[i]).BaseObject);
    ...
    if m_TargetCret <> nil then begin
      if (m_TargetCret.m_PEnvir <> m_PEnvir) or (m_TargetCret.m_boDeath) or (m_TargetCret.m_boGhost) then begin
        DelTargetCreat();
      end else
      if Integer(GetTickCount - m_dwHitTick) >= m_nNextHitTime then begin
        m_dwHitTick := GetTickCount();
