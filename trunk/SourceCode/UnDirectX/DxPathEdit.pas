...
type
  ...
  TDPointArr = array{$IFDEF DelphiX_Delphi3} [0..0]{$ENDIF} of TDPoint;
...
{$ENDIF}
    tmpPointArr: {$IFDEF DelphiX_Delphi3}PDPointArr{$ELSE}TDPointArr{$ENDIF};
    X0, Y0, LX, LY: Integer;
    IsDownNow: Boolean;
...
begin
  ...
  if FClicked and (ssRight in Shift) and Assigned(LastShape) then begin
    M := Pane.ScreenToClient({$IFDEF DelphiX_Delphi3}Point(X, Y){$ELSE}Mouse.CursorPos{$ENDIF});
...
begin
  ...
  Cursor := {$IFDEF DelphiX_Delphi3}crSIZE{$ELSE}crSizeAll{$ENDIF};
end;
...
{$ENDIF}
        ...
        ii := 0; v := {$IFDEF DelphiX_Delphi3}0{$ELSE}Low(tmpPointArr){$ENDIF};
...
begin
  if btnLine.Down then begin
    C := 0;
    if {$IFDEF DelphiX_Delphi3}tmpPointArrSize{$ELSE}Length(tmpPointArr){$ENDIF} = 2 then begin
