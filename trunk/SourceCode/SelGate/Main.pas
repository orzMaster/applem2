...
begin
  ...
  if IsBlockIP(sRemoteIPaddr) then begin
    Inc(nCCHeadOffCount);
    MainOutMessage('Filter connection: ' + sRemoteIPaddr, 3);
    Socket.Close;
    Exit;
  end;
      ...
      if UserSession.Socket <> nil then begin
        if not UserSession.boConnectCheck then begin
          if (GetTickCount - UserSession.dwUserTimeOutTick) > dwConnectTimeOut then begin
            MainOutMessage('Air connection:  ' + UserSession.sRemoteIPaddr, 3);
            {UserSession.Socket.Close;
            UserSession.Socket := nil;
            UserSession.SocketHandle := -1;
            UserSession.MsgList.Clear; }
      ...
      if (length(sCode) >= DEFBLOCKSIZE + 1) then begin
        if boCheckClientMsg then
          ...
          if (Pos(MG_CodeHead, sCode) > 0) or
            ...
            MainOutMessage('#No attack: ' + UserSession.sRemoteIPaddr, 3);
            AttackDispose(UserSession.sRemoteIPaddr, UserSession.Socket);
            sMsg := '';
            break;
          end
...
begin
  try
    MainOutMessage('Starting SelGate Server.', 0);
    ...
    MainOutMessage('SelGate Server startup complete.', 0);
...
var
  ...
  //IPList: TList;
begin
  MainOutMessage('Stopping SelGate Server...', 3);
  ...
  MainOutMessage('SelGate Server stop complete.', 3);
end;
...
begin
  if Application.MessageBox('Are you sure to stop the service?',
    'Confirmation',
...
begin
  if Application.MessageBox('Are you sure you want to clear the log information?',
    'Confirmation',
...
begin
  if boOpen then begin
    ...
    MainOutMessage('CC attack is detected, Started anit Defense.', 0);
  end else begin
