...
begin
  if UserSocketClientConnected then begin
    if GetModule(g_nGatePort) then
      UpDateModule(g_nGatePort, 'Character Gateway', User_sRemoteAddress + ':' +
        IntToStr(User_nRemotePort) + ' ¡ú ' + User_sRemoteAddress + ':' +
  ...
  if IDSocketConnected then begin
    if GetModule(nIDServerPort) then
      UpDateModule(nIDServerPort, sServerName, ID_sRemoteAddress + ':' +
        IntToStr(ID_nRemotePort) + ' ¡ú ' + ID_sRemoteAddress + ':' +
  ...
  if ServerSocketClientConnected then begin
    if GetModule(nServerPort) then
      UpDateModule(nServerPort, 'GameCenter', Server_sRemoteAddress + ':' +
        IntToStr(Server_nRemotePort) + ' ¡ú ' + Server_sRemoteAddress + ':' +
  ...
  {if DataManageSocketClientConnected then begin
    if GetModule(nDataManagePort) then
      UpDateModule(nDataManagePort, 'Data Management', DataManage_sRemoteAddress + ':' + IntToStr(DataManage_nRemotePort) + ' ¡ú ' + DataManage_sRemoteAddress + ':' + IntToStr(nServerPort), '')
    else AddModule(nDataManagePort, 'Data Management', DataManage_sRemoteAddress + ':' + IntToStr(DataManage_nRemotePort) + ' ¡ú ' + DataManage_sRemoteAddress + ':' + IntToStr(nServerPort), '');
  end else begin
...
begin
  ...
  if HumChrDB.m_Header.sDesc <> HumDB.sDBHeaderDesc then begin
    ShowMessage('Hum.DB File version does not meet the requirements ' + #13#10 + 'File Version£º' + HumChrDB.m_Header.sDesc + #13#10 + 'Required Version£º' + HumDB.sDBHeaderDesc);
    Exit;
  end;
  HumDataDB := TFileDB.Create(sDataDBFilePath + 'Mir.DB');
  if HumDataDB.m_Header.sDesc <> HumDB.sDBHeaderDesc then begin
    ShowMessage('Hum.DB File version does not meet the requirements ' + #13#10 + 'File Version£º' + HumDataDB.m_Header.sDesc + #13#10 + 'Required Version£º' + HumDB.sDBHeaderDesc);
    Exit;
  end;
