...
begin
  ...
  else if Sender = ButtonCancel then begin
    Close();
  end;
end;
...
begin
  ...
  if not IsIPaddr(m_RouteInfo.sSelGateIP) then begin
    MessageBox(Handle, 'SelGate IP error!!!','Error message', MB_OK +
      MB_ICONERROR);
  ...
  if not IsIPaddr(sGameGateIP) then begin
    MessageBox(Handle, 'Game Gateway IP error!!!',' Error message', MB_OK +
      MB_ICONERROR);
    EditGateIPaddr1.SetFocus;
    Exit;
  end;
  if nGameGatePort <= 0 then begin
    MessageBox(Handle, 'Game Gateway Port error!!!',' Error message', MB_OK +
      MB_ICONERROR);
  ...
  nGameGatePort := StrToIntDef(EditGatePort2.Text, 0);
  ...
  nGameGatePort := StrToIntDef(EditGatePort3.Text, 0);
  if (not IsIPaddr(sGameGateIP)) or (nGameGatePort <= 0) then begin
    Exit;
  end;
  ...
  nGameGatePort := StrToIntDef(EditGatePort4.Text, 0);
  ...
  nGameGatePort := StrToIntDef(EditGatePort5.Text, 0);
  if (not IsIPaddr(sGameGateIP)) or (nGameGatePort <= 0) then begin
    Exit;
  end;
  ...
  nGameGatePort := StrToIntDef(EditGatePort6.Text, 0);
  if (not IsIPaddr(sGameGateIP)) or (nGameGatePort <= 0) then
    begin
    Exit;
  end;
  ...
  nGameGatePort := StrToIntDef(EditGatePort7.Text, 0);
  if (not IsIPaddr(sGameGateIP)) or (nGameGatePort <= 0) then
    begin
    Exit;
  end;
  ...
  nGameGatePort := StrToIntDef(EditGatePort8.Text, 0);
  if (not IsIPaddr(sGameGateIP)) or (nGameGatePort <= 0) then
    begin
    Exit;
  end;
