...
interface
uses
  Windows, Classes, Graphics, SysUtils, TDX9Textures, HUtil32, WIL, DES;
...
begin
  ...
  if Result then begin
    ...
    if FCanEncry then begin
      ...
      {raise Exception.Create(strPas(@sEnStr[0]));}
  ...
  if index < FIndexList.Count then begin
    ...
    try
      if FFileStream.Read(Buffer^, ReadSize) = ReadSize then begin
        ...
        Texture.Active := True;
        Result := CopyImageDataToTexture(Buffer, Texture, imginfo.DXInfo.nWidth, imgInfo.DXInfo.nHeight, imginfo.boEncrypt, GetFormatBitLen(imginfo.btImageFormat));
      end;
    finally
      FreeMem(Buffer);
    end;
    ...
    try
      if FFileStream.Read(Buffer^, ReadSize) = ReadSize then begin
        ...
        if imginfo.boEncrypt then begin
          ...
          Try
            if DecodeRLE(Buffer, DecodeBuffer, imginfo.DXInfo.nWidth * imgInfo.DXInfo.nHeight * bitLength, bitLength) then
            begin
              ...
              begin
                ...
                for Y := 0 to Result.Height - 1 do begin
                  ...
                  wReadBuffer := @DecodeBuffer[Y * Result.Width * bitLength];
                  for X := 0 to Result.Width - 1 do begin
                    case imginfo.btImageFormat of
                      ...
                      WILFMT_R5G6B5: begin
                        if wReadBuffer^ = 0 then wWriteBuffer^ := 2113
              ...
              begin
                ...
                for Y := 0 to Result.Height - 1 do begin
                  ...
                  dwReadBuffer := @DecodeBuffer[Y * Result.Width * bitLength];
          ...
          begin
            ...
            for Y := 0 to Result.Height - 1 do begin
              wWriteBuffer := Result.ScanLine[Y];
              wReadBuffer := @Buffer[Y * Result.Width * bitLength];
              for X := 0 to Result.Width - 1 do begin
                case imginfo.btImageFormat of
                  WILFMT_A4R4G4B4: wWriteBuffer^ := AlphaBlend(wReadBuffer^);
                  WILFMT_A1R5G5B5: wWriteBuffer^ := Blend(wReadBuffer^);
          ...
          begin
            ...
            for Y := 0 to Result.Height - 1 do begin
              dwWriteBuffer := Result.ScanLine[Y];
              dwReadBuffer := @Buffer[Y * Result.Width * bitLength];
              CopyMemory(dwWriteBuffer, dwReadBuffer, Result.Width * SizeOf(LongWord));
            end;
          end;
        end;
