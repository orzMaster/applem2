...
begin
  FAngle := Reader.{$IFNDEF DelphiX_Spt3}ReadSingle{$ELSE}ReadFloat{$ENDIF};
end;

procedure TImageSprite.WriteAngle(Writer: TWriter);
begin
  Writer.{$IFNDEF DelphiX_Spt3}WriteSingle{$ELSE}WriteFloat{$ENDIF}(FAngle);
end;
...
begin
  ...
  if not Result then {$IFDEF DelphiX_Spt5}FreeAndNil(Names){$ELSE}begin Names.Free; names := nil end{$ENDIF};
end;
