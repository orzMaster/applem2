...
begin
  ...
  FSpokes := 255; Spokes.Position := FSpokes; {$IFNDEF DelphiX_Delphi3} Spokes.Smooth := True; {$ENDIF}
  FRaHUE := 1; RaHUE.Position := FRaHUE; {$IFNDEF DelphiX_Delphi3} RaHUE.Smooth := True; {$ENDIF}
  FCentr := 18; Centr.Position := FCentr; {$IFNDEF DelphiX_Delphi3} Centr.Smooth := True; {$ENDIF}
  LSpokes.Caption := Format('Spokes (%d)', [FSpokes]);
  LRaHUE.Caption := Format('Random HUE (%d)', [FRaHUE]);
  LCentr.Caption := Format('Nova radius (%d)', [FCentr]);
  Calculate;
