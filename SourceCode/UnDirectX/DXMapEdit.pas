...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls, Buttons, {$IFDEF DelphiX_Spt4}ImgList, {$ENDIF}
  Grids, Menus, DXMapEditProperties, Spin,
  DXSprite, DXClass, DXDraws, DIB, DirectX;
...
//  end;
  ...
  public
    ...
    function LoadSplittedImage(ClearContentOfMapArea: Boolean{$IFDEF DelphiX_Spt4} = False{$ENDIF}): Boolean;
...
{$ENDIF}
    ...
    if not DXBackgroundSprite.IsMapEmpty then
      for i := 0 to oldw - 1 do
        for j := 0 to oldh - 1 do
          oldmap{$IFNDEF DelphiX_Spt4}^{$ENDIF} [i, j] := DXBackgroundSprite.Map[i, j];
    {size realocation}
    with DXBackgroundSprite do begin
    ...
    for i := 0 to oldw - 1 do
      for j := 0 to oldh - 1 do
        DXBackgroundSprite.Map[i, j] := oldmap{$IFNDEF DelphiX_Spt4}^{$ENDIF} [i, j];
{$IFNDEF DelphiX_Spt4}
  finally
    FreeMem(oldmap)
  end;
...
begin
  with PicturesToChip.Canvas do begin
    if not (odSelected in State) then
      if Odd(Index) then Brush.Color := {$IFDEF DelphiX_Delphi6}clMoneyGreen{$ELSE}clGreen{$ENDIF}
...
begin
  if ImageToSet.ItemIndex <> -1 then begin
...
begin
  ...
  if (Button = mbLeft) and (DragPositionCol >= 0) and (DragPositionRow >= 0) then begin
    { Begin dragging after mouse has moved 4 pixels }
    MapArea.BeginDrag(False{$IFDEF DelphiX_Delphi4}, 2{$ENDIF});
  end;
end;
...
var
  i, j: Integer;
begin
  if PicturesToChip.ItemIndex <> -1 then begin
    with DXBackgroundSprite do
...
begin
  ...
  with DXBackgroundSprite do
    for i := 0 to MapArea.ColCount - 1 do
      for j := 0 to MapArea.RowCount - 1 do begin
        Chips[i, j] := 0;
        CollisionMap[i, j] := False;
        Map[i, j] := MapType;
      end;
...
begin
  MouseMove(Shift, X, Y);
  MapArea.MouseToCell(X, Y, FocusPopUpPositionCol, FocusPopUpPositionRow);
end;
