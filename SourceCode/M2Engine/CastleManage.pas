...
begin
  ...
  if CurCastle.m_MasterGuild = nil then
    ...
    EditOwenGuildName.Text := CurCastle.m_MasterGuild.m_sGuildName;
  ...
  if CurCastle.m_MasterGuild <> nil then
    EditCastleOfGuild.Text := CurCastle.m_MasterGuild.m_sGuildName
  else
...
begin
  ...
  for i := 0 to g_GuildManager.m_GuildList.Count - 1 do begin
    ...
    if CompareText(sGuildName, Guild.m_sGuildName) = 0 then begin
      Result := Guild;
      break;
    end;
  end;
...
function TfrmCastleManage.AddAttackSabukWallOfGuild(sGuildName: string;
  AttackSabukWall: TDate): Boolean;
var
...
function TfrmCastleManage.IsAttackSabukWallOfGuild(sGuildName: string;
  AttackDate: TDate): Boolean;
var
  i: Integer;
  ListItem: TListItem;
  AttackerInfo: pTAttackerInfo;
begin
  ...
  for i := 0 to ListViewAttackSabukWall.Items.Count - 1 do begin
    ...
    if (CompareText(sGuildName, AttackerInfo.sGuildName) = 0) and
      (AttackerInfo.AttackDate = AttackDate) then begin
      Result := True;
      break;
    end;
  end;
...
function TfrmCastleManage.ChgAttackSabukWallOfGuild(sGuildName: string;
  AttackSabukWall: TDate): Boolean;
var
  AttackerInfo: pTAttackerInfo;
  Guild: TGuild;
...
begin
  ...
  if Application.MessageBox(PChar('Are you sure to delete this guild application for siege?' + #10#10 +
    'Guild name£º' + SelAttackGuildInfo.sGuildName + #10 +
    'Siege time£º' + DateToStr(SelAttackGuildInfo.AttackDate)), 'Confirmation',
